...
class MysqlPipeline(object):
    ...
    def process_item(self, item, spider):
        if spider.name == 'get_xpath':
            ...
            site_list_sel_sql = """select site_list from bd_intro_bj where id = %s"""
            spider.cursor.execute(site_list_sel_sql, item["id"])
            result = spider.cursor.fetchone()
            if result is not None:
            ...
            dict_sel_sql = """select dict from bd_intro_bj where id = %s"""
            spider.cursor.execute(dict_sel_sql, item["id"])
            result = spider.cursor.fetchone()
            if result is not None:
                # 如果有此id，从数据库中获取intro，tuple类型
                dict = list(result)
            ...
            intro_sql = """replace into bd_intro_bj (id, quan_cheng, site_list, dict) values (%s, %s, %s, %s)"""
            args = (item["id"], item["quan_cheng"], site_list, dict)
            spider.cursor.execute(intro_sql, args)
            spider.conn.commit()
